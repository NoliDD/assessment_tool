...
# === GP verticals list (centralized) ===
try:
    # Prefer a repo module if present (e.g., config/verticals.py)
    from config.verticals import ALL_VERTICALS  # type: ignore
except Exception:
    # Fallback to local packaged list
    try:
        from config_verticals import ALL_VERTICALS  # generated helper
    except Exception:
        ALL_VERTICALS = [
            "CnG","Alcohol","Office","Home Improvement","Beauty",
            "Sports","Electronics","Pets","Party","Paint","Shoes"
        ]
\n
# Ensure full_report carries the selected vertical and JSON rules path
if isinstance(full_report, dict):
    full_report.setdefault("config", {})
    # Let env or app config override; otherwise hint a default project path
    full_report["config"].setdefault("sku_coverage_json", "config/sku_coverage_rules.json")
    # Provide the selected vertical so Final Summary can apply overrides
    full_report.setdefault("vertical", st.session_state.get("vertical"))
\n\n\1
\1
\1
\1
...
def reorder_columns_for_readability(df, is_nexla):
    ...
    groups = [
        ...
        ['IS_ALCOHOL', 'IS_CBD', 'RestrictedItemIssues?'],
